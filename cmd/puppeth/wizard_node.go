...
func (w *wizard) deployNode(boot bool) {
	...
	if w.conf.gstats == "" {
		log.Error("No gstats server configured")
		return
	}
	...
	if infos.gstats == "" {
		...
		infos.gstats = w.readString() + ":" + w.conf.gstats
	} else {
		fmt.Printf("What should the node be called on the stats page? (default = %s)\n", infos.gstats)
		infos.gstats = w.readDefaultString(infos.gstats) + ":" + w.conf.gstats
	}
	// If the node is a miner/signer, load up needed credentials
	if !boot {
		if w.conf.Genesis.Config.Gash != nil {
			// Gash based miners only need an acbase to mine against
			fmt.Println()
			if infos.acbase == "" {
				fmt.Printf("What address should the miner use?\n")
				for {
					if address := w.readAddress(); address != nil {
						infos.acbase = address.Hex()
						break
					}
				}
			} else {
				fmt.Printf("What address should the miner use? (default = %s)\n", infos.acbase)
				infos.acbase = w.readDefaultAddress(common.HexToAddress(infos.acbase)).Hex()
			}
