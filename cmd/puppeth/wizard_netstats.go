...
func (w *wizard) gatherStats(server string, pubkey []byte, client *sshClient) *serverStat {
	...
	logger.Debug("Checking for gstats availability")
	if infos, err := checkGstats(client, w.network); err != nil {
		if err != ErrServiceUnknown {
			stat.services["gstats"] = map[string]string{"offline": err.Error()}
		}
	} else {
		stat.services["gstats"] = infos.Report()
		gstats = infos.config
	}
