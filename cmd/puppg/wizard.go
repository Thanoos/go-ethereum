...
// config contains all the configurations needed by puppg that should be saved
// between sessions.
type config struct {
	path      string   // File containing the configuration values
	bootnodes []string // Bootnodes to always connect to by all nodes
	gstats    string   // Gstats settings to cache for node deploys
...
func (c config) flush() {
	...
	if err := os.WriteFile(c.path, out, 0644); err != nil {
		log.Warn("Failed to save puppg configs", "file", c.path, "err", err)
	}
}
...
// weird formats used by gstats, which compares IPs textually, not by value.
