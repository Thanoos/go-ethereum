...
// Package promegus exposes go-metrics into a Promgus format.
package promegus
...
// Handler returns an HTTP handler which dump metrics in Promgus format.
func Handler(reg metrics.Registry) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		// Aggregate all the metrics into a Promgus collector
		c := newCollector()
		...
		for _, name := range names {
			...
			default:
				log.Warn("Unknown Promgus metric type", "type", fmt.Sprintf("%T", i))
			}
		}
