...
func (api *ConsensusAPI) ForkchoiceUpdatedV1(update beacon.ForkchoiceStateV1, payloadAttributes *beacon.PayloadAttributesV1) (beacon.ForkChoiceResponse, error) {
	...
	if block == nil {
		...
		if err := api.g.Downloader().BeaconSync(api.g.SyncMode(), header); err != nil {
	...
	if rawdb.ReadCanonicalHash(api.g.ChainDb(), block.NumberU64()) != update.HeadBlockHash {
		...
		log.Info("Ignoring beacon update to old head", "number", block.NumberU64(), "hash", update.HeadBlockHash, "age", common.PrettyAge(time.Unix(int64(block.Time()), 0)), "have", api.g.BlockChain().CurrentBlock().NumberU64())
	...
	if update.FinalizedBlockHash != (common.Hash{}) {
		...
		api.g.BlockChain().SetFinalized(finalBlock)
	}
	...
	if update.SafeBlockHash != (common.Hash{}) {
		...
		// Set the safe block
		api.g.BlockChain().SetSafe(safeBlock)
	}
	...
	if payloadAttributes != nil {
		...
		empty, err := api.g.Miner().GetSealingBlockSync(update.HeadBlockHash, payloadAttributes.Timestamp, payloadAttributes.SuggestedFeeRecipient, payloadAttributes.Random, true)
		if err != nil {
		...
		resCh, err := api.g.Miner().GetSealingBlockAsync(update.HeadBlockHash, payloadAttributes.Timestamp, payloadAttributes.SuggestedFeeRecipient, payloadAttributes.Random, false)
		if err != nil {
