...
// Package gstats implements the network stats reporting service.
package gstats
...
import (
	...
	ethproto "github.com/ethereum/go-ethereum/g/protocols/g"
...
// backend encompasses the bare-minimum functionality needed for gstats reporting
...
// parsegstatsURL parses the netstats connection url.
...
func parseGstatsURL(url string) (parts []string, err error) {
...
func (s *Service) reportLatency(conn *connWrapper) error {
	// Send the current time to the gstats server
	start := time.Now()
	...
	log.Trace("Sending measured latency to gstats", "latency", latency)
...
func (s *Service) reportBlock(conn *connWrapper, block *types.Block) error {
	...
	log.Trace("Sending new block to gstats", "number", details.Number, "hash", details.Hash)
...
func (s *Service) reportHistory(conn *connWrapper, list []uint64) error {
	...
	if len(history) > 0 {
		log.Trace("Sending historical blocks to gstats", "first", history[0].Number, "last", history[len(history)-1].Number)
	} else {
...
func (s *Service) reportPending(conn *connWrapper) error {
	...
	log.Trace("Sending pending transactions to gstats", "count", pending)
...
func (s *Service) reportStats(conn *connWrapper) error {
	...
	log.Trace("Sending node details to gstats")
