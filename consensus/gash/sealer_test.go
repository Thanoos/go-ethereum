...
func TestRemoteNotify(t *testing.T) {
	...
	// Create the custom gash engine.
	gash := NewTester([]string{server.URL}, false)
	defer gash.Close()
	...
	gash.Seal(nil, block, nil, nil)
	select {
	case work := <-sink:
		if want := gash.SealHash(header).Hex(); work[0] != want {
...
func TestRemoteNotifyFull(t *testing.T) {
	...
	gash := New(config, []string{server.URL}, false)
	defer gash.Close()
...
func TestRemoteMultiNotify(t *testing.T) {
	...
	// Create the custom gash engine.
	gash := NewTester([]string{server.URL}, false)
	gash.config.Log = testlog.Logger(t, log.LvlWarn)
	defer gash.Close()
	...
	for i := 0; i < cap(sink); i++ {
		...
		block := types.NewBlockWithHeader(header)
		gash.Seal(nil, block, results, nil)
	}
...
func TestStaleSubmission(t *testing.T) {
	...
	for id, c := range testcases {
		for _, h := range c.headers {
			gash.Seal(nil, types.NewBlockWithHeader(h), results, nil)
		}
		if res := api.SubmitWork(fakeNonce, gash.SealHash(c.headers[c.submitIndex]), fakeDigest); res != c.submitRes {
		...
		case <-time.NewTimer(time.Second).C:
			t.Errorf("case %d fetch gash result timeout", id+1)
		}
	}
}
