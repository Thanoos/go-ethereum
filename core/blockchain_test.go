...
func testFork(t *testing.T, blockchain *BlockChain, i, n int, full bool, comparator func(td1, td2 *big.Int)) {
	...
	genDb, _, blockchain2, err := newCanonical(gash.NewFaker(), i, full)
	if err != nil {
	...
	if full {
		blockChainB = makeBlockChain(blockchain2.chainConfig, blockchain2.CurrentBlock(), n, gash.NewFaker(), genDb, forkSeed)
		if _, err := blockchain2.InsertChain(blockChainB); err != nil {
			t.Fatalf("failed to insert forking chain: %v", err)
		}
	} else {
		headerChainB = makeHeaderChain(blockchain2.chainConfig, blockchain2.CurrentHeader(), n, gash.NewFaker(), genDb, forkSeed)
...
func TestLastBlock(t *testing.T) {
	genDb, _, blockchain, err := newCanonical(gash.NewFaker(), 0, true)
	if err != nil {
		t.Fatalf("failed to create pristine chain: %v", err)
	}
	defer blockchain.Stop()

	blocks := makeBlockChain(blockchain.chainConfig, blockchain.CurrentBlock(), 1, gash.NewFullFaker(), genDb, 0)
...
func testExtendCanonical(t *testing.T, full bool) {
	...
	_, _, processor, err := newCanonical(gash.NewFaker(), length, full)
	if err != nil {
...
func testBrokenChain(t *testing.T, full bool) {
	...
	genDb, _, blockchain, err := newCanonical(gash.NewFaker(), 10, full)
	if err != nil {
		t.Fatalf("failed to make new canonical chain: %v", err)
	}
	defer blockchain.Stop()
	...
	if full {
		chain := makeBlockChain(blockchain.chainConfig, blockchain.CurrentBlock(), 5, gash.NewFaker(), genDb, forkSeed)[1:]
		if err := testBlockChainImport(chain, blockchain); err == nil {
			t.Errorf("broken block chain not reported")
		}
	} else {
		chain := makeHeaderChain(blockchain.chainConfig, blockchain.CurrentHeader(), 5, gash.NewFaker(), genDb, forkSeed)[1:]
...
func testReorg(t *testing.T, first, second []int64, td int64, full bool) {
	...
	genDb, _, blockchain, err := newCanonical(gash.NewFaker(), 0, full)
	if err != nil {
		t.Fatalf("failed to create pristine chain: %v", err)
	}
	defer blockchain.Stop()
	...
	easyBlocks, _ := GenerateChain(params.TestChainConfig, blockchain.CurrentBlock(), gash.NewFaker(), genDb, len(first), func(i int, b *BlockGen) {
		b.OffsetTime(first[i])
	})
	diffBlocks, _ := GenerateChain(params.TestChainConfig, blockchain.CurrentBlock(), gash.NewFaker(), genDb, len(second), func(i int, b *BlockGen) {
...
func testBadHashes(t *testing.T, full bool) {
	...
	if full {
		blocks := makeBlockChain(blockchain.chainConfig, blockchain.CurrentBlock(), 3, gash.NewFaker(), genDb, 10)
		...
		headers := makeHeaderChain(blockchain.chainConfig, blockchain.CurrentHeader(), 3, gash.NewFaker(), genDb, 10)
...
func testReorgBadHashes(t *testing.T, full bool) {
	// Create a pristine chain and database
	genDb, gspec, blockchain, err := newCanonical(gash.NewFaker(), 0, full)
	if err != nil {
		t.Fatalf("failed to create pristine chain: %v", err)
	}
	// Create a chain, import and ban afterwards
	headers := makeHeaderChain(blockchain.chainConfig, blockchain.CurrentHeader(), 4, gash.NewFaker(), genDb, 10)
	blocks := makeBlockChain(blockchain.chainConfig, blockchain.CurrentBlock(), 4, gash.NewFaker(), genDb, 10)
	...
	ncm, err := NewBlockChain(blockchain.db, nil, gspec, nil, gash.NewFaker(), vm.Config{}, nil, nil)
	if err != nil {
...
func testInsertNonceError(t *testing.T, full bool) {
	for i := 1; i < 25 && !t.Failed(); i++ {
		...
		if full {
			blocks := makeBlockChain(blockchain.chainConfig, blockchain.CurrentBlock(), i, gash.NewFaker(), genDb, 0)

			failAt = rand.Int() % len(blocks)
			failNum = blocks[failAt].NumberU64()

			blockchain.engine = gash.NewFakeFailer(failNum)
			failRes, err = blockchain.InsertChain(blocks)
		} else {
			headers := makeHeaderChain(blockchain.chainConfig, blockchain.CurrentHeader(), i, gash.NewFaker(), genDb, 0)
...
func TestFastVsFullChains(t *testing.T) {
	...
	_, blocks, receipts := GenerateChainWithGenesis(gspec, gash.NewFaker(), 1024, func(i int, block *BlockGen) {
	...
	archive, _ := NewBlockChain(archiveDb, nil, gspec, nil, gash.NewFaker(), vm.Config{}, nil, nil)
	defer archive.Stop()
	...
	fast, _ := NewBlockChain(fastDb, nil, gspec, nil, gash.NewFaker(), vm.Config{}, nil, nil)
	defer fast.Stop()
	...
	ancient, _ := NewBlockChain(ancientDb, nil, gspec, nil, gash.NewFaker(), vm.Config{}, nil, nil)
	defer ancient.Stop()
...
func TestLightVsFastVsFullChainHeads(t *testing.T) {
	...
	archive, _ := NewBlockChain(archiveDb, &archiveCaching, gspec, nil, gash.NewFaker(), vm.Config{}, nil, nil)
	if n, err := archive.InsertChain(blocks); err != nil {
		t.Fatalf("failed to process block %d: %v", n, err)
	}
	defer archive.Stop()
	...
	lightDb := makeDb()
	defer lightDb.Close()
	light, _ := NewBlockChain(lightDb, nil, gspec, nil, gash.NewFaker(), vm.Config{}, nil, nil)
...
func TestChainTxReorgs(t *testing.T) {
	...
	_, chain, _ := GenerateChainWithGenesis(gspec, gash.NewFaker(), 3, func(i int, gen *BlockGen) {
		switch i {
		case 0:
	...
	_, chain, _ = GenerateChainWithGenesis(gspec, gash.NewFaker(), 5, func(i int, gen *BlockGen) {
		switch i {
		case 0:
...
func TestLogReorgs(t *testing.T) {
	...
	blockchain, _ := NewBlockChain(rawdb.NewMemoryDatabase(), nil, gspec, nil, gash.NewFaker(), vm.Config{}, nil, nil)
	defer blockchain.Stop()

	rmLogsCh := make(chan RemovedLogsEvent)
	blockchain.SubscribeRemovedLogsEvent(rmLogsCh)
	_, chain, _ := GenerateChainWithGenesis(gspec, gash.NewFaker(), 2, func(i int, gen *BlockGen) {
		if i == 1 {
...
func TestSideLogRebirth(t *testing.T) {
	...
	genDb, sideChain, _ := GenerateChainWithGenesis(gspec, gash.NewFaker(), 2, func(i int, gen *BlockGen) {
		if i == 1 {
	...
	newBlocks, _ := GenerateChain(gspec.Config, sideChain[len(sideChain)-1], gash.NewFaker(), genDb, 1, func(i int, gen *BlockGen) {})
	if _, err := blockchain.InsertChain(newBlocks); err != nil {
		t.Fatalf("failed to insert forked chain: %v", err)
	}
...
func TestReorgSideEvent(t *testing.T) {
	...
	_, replacementBlocks, _ := GenerateChainWithGenesis(gspec, gash.NewFaker(), 4, func(i int, gen *BlockGen) {
...
func TestCanonicalBlockRetrieval(t *testing.T) {
	_, gspec, blockchain, err := newCanonical(gash.NewFaker(), 0, true)
	if err != nil {
		t.Fatalf("failed to create pristine chain: %v", err)
	}
	defer blockchain.Stop()

	_, chain, _ := GenerateChainWithGenesis(gspec, gash.NewFaker(), 10, func(i int, gen *BlockGen) {})
...
func TestEIP155Transition(t *testing.T) {
	...
	genDb, blocks, _ := GenerateChainWithGenesis(gspec, gash.NewFaker(), 4, func(i int, block *BlockGen) {
		var (
	...
	blocks, _ = GenerateChain(config, blocks[len(blocks)-1], gash.NewFaker(), genDb, 4, func(i int, block *BlockGen) {
		var (
			tx      *types.Transaction
			err     error
			basicTx = func(signer types.Signer) (*types.Transaction, error) {
...
func TestEIP161AccountRemoval(t *testing.T) {
	...
	_, blocks, _ := GenerateChainWithGenesis(gspec, gash.NewFaker(), 3, func(i int, block *BlockGen) {
		var (
			tx     *types.Transaction
			err    error
			signer = types.LatestSigner(gspec.Config)
...
func TestInsertReceiptChainRollback(t *testing.T) {
	...
	ancientChain, _ := NewBlockChain(ancientDb, nil, gspec, nil, gash.NewFaker(), vm.Config{}, nil, nil)
...
func TestTransactionIndices(t *testing.T) {
	...
	_, blocks, receipts := GenerateChainWithGenesis(gspec, gash.NewFaker(), 128, func(i int, block *BlockGen) {
		tx, err := types.SignTx(types.NewTransaction(block.TxNonce(address), common.Address{0x00}, big.NewInt(1000), params.TxGas, block.header.BaseFee, nil), signer, key)
		if err != nil {
			panic(err)
		}
	...
	for _, l := range limit {
		...
		l := l
		chain, err := NewBlockChain(ancientDb, nil, gspec, nil, gash.NewFaker(), vm.Config{}, nil, &l)
		if err != nil {
			t.Fatalf("failed to create tester chain: %v", err)
		}
...
//     with 0.001 ac (thus created but no code)
...
func TestEIP1559Transition(t *testing.T) {
	var (
		...
		addr1   = crypto.PubkeyToAddress(key1.PublicKey)
		addr2   = crypto.PubkeyToAddress(key2.PublicKey)
		funds   = new(big.Int).Mul(common.Big1, big.NewInt(params.AC))
		gspec   = &Genesis{
			Config: params.AllGashProtocolChanges,
			Alloc: GenesisAlloc{
				addr1: {Balance: funds},
	...
	expected := new(big.Int).Add(
		...
		gash.ConstantinopleBlockReward,
	)
